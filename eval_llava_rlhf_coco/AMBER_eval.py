...
def get_args():
    ...
    parser.add_argument("--word_association", type=str, default='./eval_llava_rlhf_coco/AMBER_data/relation.json')
    parser.add_argument("--safe_words", type=str, default='./eval_llava_rlhf_coco/AMBER_data/safe_words.txt')
    parser.add_argument("--inference_data", type=str)
    parser.add_argument("--annotation", type=str, default='./eval_llava_rlhf_coco/AMBER_data/annotations.json')
    parser.add_argument("--metrics", type=str, default='./eval_llava_rlhf_coco/AMBER_data/metrics.txt')
...
def main(args):
    ...
    inference_data = []
    with open(args.inference_data, 'r', encoding='utf-8') as f:
        for line in f:
    ...
    # for i in range(len(inference_data)):
        ...
        if ground_truth[id-1]['type'] == 'generative':
            ...
            # print(f"{sum(safe_list[-safe_len:])/len(safe_list[-safe_len:])}")
            # # if sum(safe_flag_list) == 0:
            # #     print(0)
            # # else:
            # #     print(1)
    ...
    for i in inference_data:
        if len(i['response']) >= 800:
            count += 1
    print(f"Repeat Ratio: {count/len(inference_data) * 100:.2f}%")
